...
public class SalesDataReader {
	...
	private static SalesDataReader brokerInstance;

	public static SalesDataReader getInstance(BlockingQueue<SalesData> blockingQueue){
		if(null== brokerInstance){
			...
			brokerInstance = new SalesDataReader(url, user, password , blockingQueue);
		}
		return brokerInstance;
	}

	private SalesDataReader(String url, String user, String password ,BlockingQueue<SalesData> blockingQueue) {
		super();
		this.url = url;
		this.user = user;
...
//			querySb.append(" FETCH FIRST 10 ROWS ONLY") ; // TODO remove when tested
			...
			while (rs.next()) {
				...
				else if (couponNo != currentCoupNo + 1) {
